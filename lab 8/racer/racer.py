...
others = [yellow_car, white_car, red_car]
others_rect = [others[0].get_rect(), others[1].get_rect(),
               others[2].get_rect()]  # ну этим можно узнать их размеры по x and y
...
class Coin: # класс для создание монеток
    def __init__(self, lists, rect, image): # передается его лист его размеры и его фотка, лучше так чем делать два класса например
...
def lose():# тут обнуляю все значение на изначальные, на самом деле мог бы создать какой то txt файл с изначальными мбмбмб
    global loser, targets, gg_x, timer_for_target, score, \
        speed_y, speed_of_gg, list_of_coin_5, list_of_coin_2,score_coins
    loser = True
    ...
    screen.blit(score_coin_text,(width//2-score_coin_text.get_rect()[2]//2,456+score_coin_text.get_rect()[3]+40))
